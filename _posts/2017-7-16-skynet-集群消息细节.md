...
```
上面的代码把长数据分割成最大为32k的数据块，然后依次插入到table中。再次回到lua层的clustersender中的push代码。通过c层代码我们知道：cluster.packpush如果发送的数据是小于32k的，那么padding就是nil，否则，返回的requst是把长数据拆分出来的头部（head），padding是一个table，里面是body和tail数据。
```
